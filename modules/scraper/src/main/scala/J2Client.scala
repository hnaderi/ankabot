...
object JdkHttpClient2 {
  ...
  /** Creates a `Client` from an `HttpClient`.
    ...
    *   A set of ignored request headers. Some headers (like Content-Length) are
  ...
  def apply[F[_]](
    ...
    def convertRequest(req: Request[F]): Resource[F, HttpRequest] =
      flow.toPublisher(req.body.chunks.map(_.toByteBuffer)).evalMap {
        publisher =>
    ...
    def convertResponse(
      ...
      Resource
        ...
        .flatMap { case (subscription, res) =>
          ...
          Resource(
            (
              F.fromEither(Status.fromInt(res.statusCode)),
              SignallingRef[F, Boolean](false)
            ).mapN { case (status, signal) =>
              Response(
                status = status,
                headers =
                  Headers(res.headers.map.asScala.flatMap { case (k, vs) =>
                ...
                body = body
                  ...
                  .flatMap(bs =>
                    Stream.fromIterator(
                      bs.iterator.asScala.map(Chunk.byteBuffer),
                      1
                    )
                  )
  ...
  private[jdkhttpclient] def defaultHttpClient[F[_]](implicit
      F: Async[F]
  ): F[HttpClient] =
    F.executor.flatMap { exec =>
      F.delay {
        ...
        builder.connectTimeout(
          Duration.ofNanos(org.http4s.client.defaults.ConnectTimeout.toNanos)
        )
  ...
  def convertHttpVersionFromHttp4s[F[_]](
    ...
    version match {
      ...
      case _ =>
        F.raiseError(new IllegalArgumentException("invalid HTTP version"))
    }
